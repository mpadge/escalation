...
load_gp_inputs <- function (results_dir, pars) {
    ...
    param_medians <- sapply (
        param_names, function (nm) median (gp_data [[nm]], na.rm = TRUE)
    )
    ...
    cli_inform ("")
    cli_h2 (col_yellow ("Construction"))
    cli_alert_info ("Building phase diagrams for: {.field {top_params}}")
...
build_phase_diagrams <- function (top_params, param_names, param_medians,
                                  all_binf, all_bsup, fit_psi, fit_tau,
                                  phase_dir) {
...
run_emulator_sobol <- function (fit_psi, param_names, all_binf, all_bsup,
                                n_sobol, batch_size, results_dir) {
    cli_inform ("")
    cli_h2 (col_yellow ("Emulation"))
    ...
    write.csv (sobol_gp, file.path (results_dir, "sobol_gp.csv"),
               row.names = FALSE)
...
inputs <- load_gp_inputs (results_dir, pars)

build_phase_diagrams (
    inputs$top_params, inputs$param_names, inputs$param_medians,
    all_binf, all_bsup, inputs$fit_psi, inputs$fit_tau,
    phase_dir
)

run_emulator_sobol (
    inputs$fit_psi, inputs$param_names, all_binf, all_bsup,
