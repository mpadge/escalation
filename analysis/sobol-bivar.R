#!/usr/bin/env Rscript
# Bivariate Sobol' variance-based sensitivity analysis for the (ε, σ) model (Stage 6).
#
# Reads top parameters from results/morris_bivar_results_psi_sigma.csv (written by
# make morris-bivar), generates a Saltelli design, runs the Rust binary, and computes
# first-order (S1) and total-order (ST) indices for psi_sigma.
#
# eta_obs is excluded from the candidate set — it was held fixed in the bivariate
# Morris to avoid collinearity with mu_sigma and therefore does not appear in the
# Morris rankings. Joint variation with mu_sigma is deferred to Stage 7.
#
# Prerequisites: install.packages(c("sensitivity", "processx", "dplyr", "cli", "ggplot2"))
# Run from project root: Rscript analysis/sobol-bivar.R
# Requires: make morris-bivar completed (results/morris_bivar_results_psi_sigma.csv)

Sys.setenv (`_R_S3_METHOD_REGISTRATION_NOTE_OVERWRITES_` = "false")
library (sensitivity)
library (processx)
library (dplyr, warn.conflicts = FALSE)
library (RcppTOML)
library (cli)
library (ggplot2)

source ("analysis/utils.R")

# ---------------------------------------------------------------------------
# Functions
# ---------------------------------------------------------------------------

select_sobol_params_bivar <- function (results_dir, all_param_names, top_n) {
    morris_file <- file.path (results_dir, "morris_bivar_results_psi_sigma.csv")
    if (file.exists (morris_file)) {
        df <- read.csv (morris_file)
        param_names <- df$param [seq_len (min (top_n, nrow (df)))]
        cli_alert_info (
            "Using top {.val {length(param_names)}} parameters from bivariate \\
            Morris (psi_sigma metric): {.field {param_names}}"
        )
    } else {
        param_names <- all_param_names
        cli_alert_warning (
            "morris_bivar_results_psi_sigma.csv not found; \\
            using all {.val {length(param_names)}} parameters"
        )
    }
    param_names
}

make_saltelli_design_bivar <- function (param_names, binf, bsup, fixed,
                                        n_sobol, results_dir) {
    p <- length (param_names)
    cli_alert_info ("Generating Saltelli design (n={n_sobol}, p={p})...")
    cli_alert_info ("Total binary calls: {n_sobol * (2 * p + 2)}")

    make_uniform <- function (n, pnames, lo, hi) {
        mat <- matrix (runif (n * length (pnames)), n, length (pnames))
        df  <- as.data.frame (mat)
        colnames (df) <- pnames
        for (nm in pnames) df [[nm]] <- lo [nm] + (hi [nm] - lo [nm]) * df [[nm]]
        df
    }

    x1    <- make_uniform (n_sobol, param_names, binf, bsup)
    x2    <- make_uniform (n_sobol, param_names, binf, bsup)
    s_obj <- sobol2007 (model = NULL, X1 = x1, X2 = x2, nboot = 100)

    n_expected <- nrow (s_obj$X)
    cli_alert_info ("Saltelli design has {.val {n_expected}} rows")

    design_full <- s_obj$X
    for (nm in names (fixed)) design_full [[nm]] <- fixed [[nm]]
    for (nm in param_names)   design_full [[nm]] <- s_obj$X [[nm]]
    design_full$theta <-
        pmax (1L, pmin (4L, as.integer (round (design_full$theta))))
    design_full$n     <- as.integer (design_full$n)
    design_full$t_max <- as.integer (design_full$t_max)

    write.csv (
        design_full,
        file.path (results_dir, "design_sobol_bivar.csv"),
        row.names = FALSE
    )
    cli_alert_info ("Wrote design_sobol_bivar.csv")
    list (s_obj = s_obj, n_expected = n_expected)
}

run_sobol_binary_bivar <- function (binary, results_dir, log_dir) {
    cli_alert_info ("Running bivariate Sobol binary...")
    result <- processx::run (
        binary,
        c (
            "sobol", "--design",
            file.path (results_dir, "design_sobol_bivar.csv"),
            "--output", file.path (results_dir, "sobol_bivar_raw.csv"),
            "--log-dir", log_dir
        ),
        echo = TRUE, error_on_status = FALSE
    )
    if (result$status != 0) {
        cli_abort ("Binary failed: {result$stderr}")
    }
}

compute_sobol_indices_bivar <- function (s_obj, results_dir, param_names) {
    raw      <- read.csv (file.path (results_dir, "sobol_bivar_raw.csv"))
    psi_vals <- raw$psi_sigma [seq (1, nrow (raw), by = 2)]
    if (any (is.na (psi_vals))) {
        cli_alert_warning (
            "{.val {sum(is.na(psi_vals))}} NA psi_sigma values replaced with 0"
        )
        psi_vals [is.na (psi_vals)] <- 0
    }

    s_obj <- tell (s_obj, psi_vals)

    results <- data.frame (
        param    = param_names,
        S1       = s_obj$S$original,
        S1_lower = s_obj$S$`min. c.i.`,
        S1_upper = s_obj$S$`max. c.i.`,
        ST       = s_obj$T$original,
        ST_lower = s_obj$T$`min. c.i.`,
        ST_upper = s_obj$T$`max. c.i.`
    )
    results <- results [order (-results$ST), ]
    write.csv (
        results,
        file.path (results_dir, "sobol_bivar_results.csv"),
        row.names = FALSE
    )
    cli_alert_info ("Sobol results ranked by ST (psi_sigma metric):")
    print (results, digits = 3)
    cli_alert_info ("Wrote {.file sobol_bivar_results.csv}")
    invisible (results)
}

plot_sobol_bivar <- function (results, results_dir) {
    plots_dir <- file.path (results_dir, "plots")
    dir.create (plots_dir, showWarnings = FALSE, recursive = TRUE)

    df <- results [order (-results$ST), ]
    df$param <- factor (df$param, levels = df$param)

    df_long <- rbind (
        data.frame (
            param = df$param, index = "S1",
            value = df$S1, lower = df$S1_lower, upper = df$S1_upper
        ),
        data.frame (
            param = df$param, index = "ST",
            value = df$ST, lower = df$ST_lower, upper = df$ST_upper
        )
    )

    p <- ggplot (df_long, aes (x = param, y = value, fill = index)) +
        geom_col (position = position_dodge (0.7), width = 0.6) +
        geom_errorbar (
            aes (ymin = lower, ymax = upper),
            position = position_dodge (0.7), width = 0.25
        ) +
        scale_fill_manual (values = c (S1 = "#2166ac", ST = "#d6604d")) +
        labs (
            title = "Sobol' sensitivity indices for psi_sigma — bivariate (ε, σ) model",
            x     = NULL,
            y     = "Sobol index",
            fill  = "Index"
        ) +
        theme_bw (base_size = 12) +
        theme (legend.position = "bottom")

    out <- file.path (plots_dir, "sobol_bivar_plot.png")
    ggsave (out, p, width = 8, height = 5, dpi = 300)
    cli_alert_info ("Saved {.file {out}}")
}

# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------

set.seed (42)

results_dir <- "results"
if (!dir.exists (results_dir)) {
    cli_abort (
        "Output directory {.file {results_dir}} not found — \\
        run {.code make morris-bivar} first"
    )
}

# Full bivariate candidate set (14 params; eta_obs excluded — held fixed in Morris)
all_param_names <- c (
    "gamma", "lambda", "alpha", "theta", "beta",
    "w_win", "b", "w_loss", "dw_obs", "dw_bridge",
    "mu_sigma", "sigma_sigma", "eta_sigma", "sigma_decay"
)

pars   <- RcppTOML::parseTOML ("defaults.toml")
pars_s <- pars$structural
pars_a <- pars$analysis

# Ranges for all candidate parameters
get_range <- function (nm, lo_default, hi_default) {
    r <- pars$ranges [[nm]]
    if (!is.null (r)) r else c (lo_default, hi_default)
}

all_ranges <- list (
    gamma       = get_range ("gamma",       1.0,   5.0),
    lambda      = get_range ("lambda",      1.0,   5.0),
    alpha       = get_range ("alpha",       0.1,   2.0),
    theta       = get_range ("theta",       1.0,   4.0),
    beta        = get_range ("beta",        0.0,   1.0),
    w_win       = get_range ("w_win",       0.0,   2.0),
    b           = get_range ("b",           0.0,   2.0),
    w_loss      = get_range ("w_loss",      0.1,   2.0),
    dw_obs      = get_range ("dw_obs",      0.0,   0.2),
    dw_bridge   = get_range ("dw_bridge",   0.0,   0.2),
    mu_sigma    = get_range ("mu_sigma",    0.5,   2.0),
    sigma_sigma = get_range ("sigma_sigma", 0.0,   0.5),
    eta_sigma   = get_range ("eta_sigma",   0.0,   0.2),
    sigma_decay = get_range ("sigma_decay", 0.001, 0.01)
)

param_names <- select_sobol_params_bivar (
    results_dir, all_param_names, pars$sobol$top_n
)
p    <- length (param_names)
binf <- setNames (
    vapply (param_names, function (nm) all_ranges [[nm]] [1L], numeric (1)),
    param_names
)
bsup <- setNames (
    vapply (param_names, function (nm) all_ranges [[nm]] [2L], numeric (1)),
    param_names
)

log_dir <- if (!is.null (pars_s$log_dir)) pars_s$log_dir else "/tmp/escalation"
dir.create (log_dir, recursive = TRUE, showWarnings = FALSE)
safe_clear_done_files (log_dir, expected_n = pars$sobol$n_sobol * (2L * p + 2L))
cli_alert_info ("Progress files will be written to {log_dir}")

fixed <- list (
    n             = as.integer (pars$sobol$n_sobol_pop),
    mu0           = pars_a$mu0,
    sigma0        = pars_a$sigma0,
    c             = pars_a$c,
    e             = pars_a$e,
    dw_coop       = pars_a$dw_coop,
    dw_sub        = pars_a$dw_sub,
    dw_excl       = pars_a$dw_excl,
    eta           = pars_a$eta,
    eta_obs       = pars_a$mid_eta_obs,
    delta_direct  = pars_a$delta_direct,
    delta_exploit = pars_a$delta_exploit,
    w_min         = pars_s$w_min,
    w_max         = pars_s$w_max,
    sigma_drift   = pars_s$sigma_drift,
    rho_contested = pars_s$rho_contested,
    eta_trauma    = pars_s$eta_trauma,
    delta         = pars_s$delta,
    t_max         = as.integer (pars_a$t_max_sobol),
    # Midpoints for non-selected original params
    gamma         = pars_a$mid_gamma,
    lambda        = pars_a$mid_lambda,
    alpha         = pars_a$mid_alpha,
    theta         = as.integer (pars_a$mid_theta),
    beta          = pars_a$mid_beta,
    w_win         = pars_a$mid_w_win,
    b             = pars_a$mid_b,
    w_loss        = pars_a$mid_w_loss,
    dw_obs        = pars_a$mid_dw_obs,
    dw_bridge     = pars_a$mid_dw_bridge,
    # Midpoints for non-selected σ-trait params
    mu_sigma      = pars_a$mid_mu_sigma,
    sigma_sigma   = pars_a$mid_sigma_sigma,
    eta_sigma     = pars_a$mid_eta_sigma,
    sigma_decay   = pars_a$mid_sigma_decay
)

binary <- "./target/release/escalation"
if (!file.exists (binary)) {
    cli_abort ("Binary not found — run 'cargo build --release'")
}

cli_inform ("")
design <- make_saltelli_design_bivar (
    param_names, binf, bsup, fixed, pars$sobol$n_sobol, results_dir
)
cli_alert_info (
    "Expected {.val {design$n_expected}} progress files — \\
    monitor with {.code make progress}."
)
cli_inform ("")
run_sobol_binary_bivar (binary, results_dir, log_dir)
cli_inform ("")
results <- compute_sobol_indices_bivar (design$s_obj, results_dir, param_names)
plot_sobol_bivar (results, results_dir)
