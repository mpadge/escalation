...
if (length(phase_files) == 0) {
  ...
  for (f in phase_files) {
    ...
    p_phase <- ggplot(df, aes(x = .data[[xvar]], y = .data[[yvar]])) +
      ...
      geom_contour(aes(z = psi_sd), colour = "white", alpha = 0.6,
...
if (file.exists("morris_results.csv")) {
  m <- read.csv("morris_results.csv") %>%
    transmute(param, value = mu_star / max(mu_star, na.rm = TRUE),
              method = "Morris mu*")
  sources[["morris"]] <- m
}
...
if (length(sources) >= 2) {
  ...
  p_sobol <- ggplot(combined,
                    aes(x = reorder(param, value, FUN = max),
                        y = value, fill = method)) +
